  ...
  const continueToNext = async () => {
    if (await trigger(stepFields[step], { shouldFocus: true })) {
      ...
      window.requestAnimationFrame(() =>
        document
          .getElementById('booking-form-heading')
  ...
  const onSubmit = async (data: BookingInput) => {
    ...
    try {
      ...
      if (caught instanceof BackendError && Object.keys(caught.fieldErrors).length) {
        const mapping: Record<string, keyof BookingInput> = {
        ...
        const errorStep = stepFields.findIndex((fields) =>
          fields.some((field) => field in formErrors)
        );
        if (errorStep >= 0) setStep(errorStep);
        showError(
          'Please review your information',
          'A few details need your attention before we can send this request.',
          formErrors
        );
  ...
  return (
    <>
      <SuccessModal
        ...
        message="Thank you. We’ll review the engagement details and contact you within 3–5 business days. Your event is not confirmed until a formal agreement is completed."
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate aria-labelledby="booking-form-heading">
            ...
            return (
              <li key={item.label} className="relative">
                <button
                  ...
                  className={cn(
                    'flex min-h-16 w-full flex-col items-center justify-center gap-1.5 rounded-lg border p-2 text-center transition sm:min-h-14 sm:flex-row sm:gap-2',
                    index === step
                ...
                >
                  <span
                    className={cn(
                      ...
                      index <= step ? 'bg-purple-600 text-white' : 'bg-neutral-200'
                    )}
                  >
                  ...
                  <span>
                    ...
                    <span className="text-xs font-semibold sm:hidden">{item.short}</span>
                  </span>
                </button>
              </li>
            );
        ...
        {step === 1 && (
          <div className="space-y-6">
            ...
            <div>
              ...
              <input
                {...register('eventDate', {
                  required: 'Choose the proposed event date.',
                  validate: (value) => value >= tomorrow() || 'Choose a future date.',
                })}
                type="date"
                min={tomorrow()}
                className={fieldClass}
            ...
            <div>
              ...
              <textarea
                {...register('eventDetails', {
                  required: 'Tell us about the event.',
                  minLength: {
                    value: 30,
                    message:
                      'Please provide at least 30 characters so our team can assess the request.',
                  },
                  maxLength: {
                    value: 2000,
                    message: 'Keep the event brief under 2,000 characters.',
                  },
                })}
                className={textAreaClass}
                placeholder="Include the event vision, theme, audience, expected attendance, requested ministry format, and any important timing."
              />
              <div className="mt-2 flex justify-end text-xs text-neutral-400">
                <span>{watch('eventDetails')?.length ?? 0}/2000</span>
              </div>
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <Controller
                name="country"
                control={control}
                rules={{ required: 'Choose the event country.' }}
                render={({ field }) => (
                  <CountrySelect
                    value={field.value}
                    onChange={field.onChange}
            ...
            <div className="flex items-start gap-3 rounded-lg bg-purple-950 px-4 py-4 text-white">
              ...
              <p className="text-xs leading-5 text-white/70">
                <strong className="block text-sm text-white">Handled confidentially</strong>Your
                details are shared only with the booking team and used to assess this request.
              </p>
            </div>
          </div>
        )}
        ...
        <ErrorModal
          ...
          onClose={closeError}
          actions={[{ label: 'Review details', onClick: closeError, variant: 'primary' }]}
        />
        <div className="mt-9 flex items-center justify-between gap-3 border-t border-neutral-100 pt-6">
          {step > 0 ? (
            <button
              ...
              className="inline-flex h-12 items-center gap-2 rounded-lg px-3 text-sm font-semibold text-neutral-600 hover:bg-neutral-100 disabled:opacity-50"
            >
              <ChevronLeft className="h-4 w-4" /> Back
            </button>
          ) : (
            <p className="hidden text-xs text-neutral-400 sm:block">
              Fields are validated before you continue.
            </p>
          )}
          {step < steps.length - 1 ? (
            <button
              ...
              className="inline-flex h-12 items-center gap-2 rounded-lg bg-purple-700 px-6 text-sm font-semibold text-white shadow-purple-cta transition hover:bg-purple-600 disabled:opacity-50"
            >
            ...
            >
              {isSubmitting ? (
                <>
                  <Loader2 className="h-4 w-4 animate-spin" /> Sending securely…
                </>
              ) : (
                <>
