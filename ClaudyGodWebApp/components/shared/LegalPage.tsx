    ...
    <div className="min-h-screen bg-neutral-50 pt-[var(--navbar-height)]">
      ...
      <div className="bg-white border-b border-neutral-100">
        <div className="max-w-[1100px] mx-auto px-6 lg:px-12 py-10">
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-neutral-400 hover:text-neutral-700 font-sans text-xs tracking-[0.12em] uppercase transition-colors duration-200 mb-6"
          >
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:gap-16">
          ...
          <aside className="lg:w-56 shrink-0">
            <div className="lg:sticky lg:top-28">
              <p className="font-display font-bold text-neutral-700 text-xs uppercase tracking-widest mb-4">
                Legal Documents
              </p>
              <nav className="flex flex-row lg:flex-col gap-2">
                {legalNav.map(({ href, label, icon: Icon }) => (
                  <Link
                    key={href}
                    href={href}
                    className={`flex items-center gap-3 px-4 py-3 rounded-xl border font-display font-semibold text-sm transition-all duration-200 ${
              ...
              <div className="px-8 py-8 space-y-0 divide-y divide-neutral-100">
                {sections.map((s, i) => (
                  <div key={i} className="py-8 first:pt-0 last:pb-0">
                    <h2 className="font-display font-bold text-neutral-900 text-xl tracking-tight mb-3">
                      {s.heading}
                    </h2>
                    <p className="font-sans text-neutral-500 text-base leading-[1.85]">{s.body}</p>
                  </div>
                ))}
              </div>
