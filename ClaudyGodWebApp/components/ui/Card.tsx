...
const cardVariants = cva('relative overflow-hidden transition-all duration-300', {
  variants: {
    variant: {
      default: ['bg-surface-elevated border border-surface-border rounded-xl', 'shadow-card'],
      elevated: ['bg-surface-overlay border border-surface-border rounded-xl', 'shadow-card-hover'],
...
export function CardHeader({ className, children, ...props }: HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn('flex flex-col gap-1.5 pb-4 border-b border-surface-border', className)}
      {...props}
    >
      {children}
    </div>
