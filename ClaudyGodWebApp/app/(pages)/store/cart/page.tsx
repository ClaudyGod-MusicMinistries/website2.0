  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen bg-cream-100 pt-[var(--navbar-height)] flex flex-col items-center justify-center gap-6 text-center px-6">
        ...
        <Link
          href="/store"
          className="mt-2 font-sans text-xs tracking-[0.18em] uppercase text-white bg-purple-600 hover:bg-purple-700 transition-colors h-11 px-8 inline-flex items-center gap-2 rounded-xl"
        >
      ...
      <div className="container-site py-16 md:py-20">
        ...
        <div className="flex items-center justify-between mb-10">
          <div className="flex items-center gap-4">
            ...
            <h1 className="font-raleway font-light text-neutral-900 text-3xl tracking-normal">
              Your Cart
            </h1>
          </div>
          <Link
            href="/store"
            className="hidden sm:inline-flex items-center gap-2 font-sans text-xs tracking-[0.15em] uppercase text-neutral-500 hover:text-purple-600 transition-colors"
          >
            <ArrowLeft className="h-3.5 w-3.5" />
            Continue Shopping
          </Link>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="lg:col-span-2">
              ...
              {items.map((item) => (
                <div key={item.id} className="p-6 flex gap-5">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-display font-semibold text-neutral-900 text-sm leading-snug mb-0.5">
                      {item.name}
                    </p>
                    <p className="font-sans text-neutral-400 text-xs mb-4 capitalize">
                    ...
                    <div className="flex items-center gap-5">
                      <div className="flex items-center rounded-xl border border-neutral-200 overflow-hidden">
                        ...
                        <span className="w-8 h-8 flex items-center justify-center border-x border-neutral-200 font-sans text-xs text-neutral-800 font-medium">
                          {item.quantity}
                        </span>
                        <button
                          onClick={() => updateQuantity(item.id, item.quantity + 1)}
            ...
            <button
              onClick={clearCart}
              className="mt-4 font-sans text-[0.6rem] tracking-[0.15em] uppercase text-neutral-400 hover:text-red-400 transition-colors"
            >
              Clear Cart
            </button>
          </div>
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-xl shadow-card-light border border-black/[0.04] p-6">
              ...
              <div className="space-y-3 mb-6">
                {items.map((item) => (
                  <div key={item.id} className="flex justify-between gap-3">
                    <p className="font-sans text-neutral-500 text-sm truncate flex-1">
                      {item.name} <span className="text-neutral-300">× {item.quantity}</span>
                    </p>
                    <p className="font-sans font-medium text-neutral-700 text-sm shrink-0">
                      {formatPrice(item.price * item.quantity)}
                    </p>
                  </div>
                ))}
              ...
              <Link
                href="/store"
                className="mt-3 w-full h-11 border border-neutral-200 hover:border-purple-300 text-neutral-500 hover:text-purple-600 font-sans text-xs tracking-[0.12em] uppercase rounded-xl transition-all duration-300 flex items-center justify-center gap-2"
              >
                <ArrowLeft className="h-3.5 w-3.5" />
                Continue Shopping
              </Link>
